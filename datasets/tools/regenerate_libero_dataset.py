...
def main(args):
    ...
    metainfo_json_dict = {}
    metainfo_json_out_path = f"./datasets/tools/libero/{args.libero_task_suite}_metainfo.json"
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for i in range(len(orig_data.keys())):
            ...
            eye_in_hand_images = []
            abs_eef = []
            agentview_images.append(_flip_agentview(obs['agentview_image']))
            ...
            abs_eef.append(np.concatenate([goal_pos, state_ori, np.array([-1.])]))
            ...
            if done:
                ...
                assert len(abs_eef) == len(agentview_images)
                # skip the fail case
                new_data_path = os.path.join(args.libero_target_dir, f"{task.name}_demo_{i}.hdf5")
                new_data_file = h5py.File(new_data_path, "w")
                new_data_file.create_dataset("observation/agentview_rgb", data=np.stack(agentview_images, axis=0))
                new_data_file.create_dataset("observation/eye_in_hand_rgb", data=np.stack(eye_in_hand_images, axis=0))
                # new_data_file.create_dataset("actions", data=actions)
                new_data_file.create_dataset("abs_action_6d", data=np.stack(abs_eef, axis=0))
                # new_data_file.create_dataset("states", data=np.stack(states, axis=0))
                # new_data_file.create_dataset("robot_states", data=np.stack(robot_states, axis=0))
                new_data_file.create_dataset("rewards", data=rewards)
                new_data_file.create_dataset("dones", data=dones)
                new_data_file.attrs['instruction'] = task_description
                new_data_file.attrs['task_name'] = task.name
                new_data_file.close()
